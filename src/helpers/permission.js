...
export const isHasPermissionForButton = (buttonType) => {
    ...
    if (!_.isEmpty(path)) {
        const permission = _.find(viewPermissions, (x) => _.isEqual(_.toLower(x.view), _.toLower(path)))
        if (!_.isNil(permission)) {
            const actions = permission.actions || []
            isHasPermission = _.some(actions, (x) => _.isEqual(x.key, buttonType) && x.value === true)
        }
    }
