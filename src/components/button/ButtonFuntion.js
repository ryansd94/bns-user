...
const ButtonFuntion = React.memo((props) => {
  ...
  const [float, setFloat] = useState(
    isFloatLeft ? " float-left" : " float-right",
  );
  ...
  const setButton = () => {
    switch (type) {
      ...
      case EButtonType.filter:
        if (open && open) setIcon("far fa-angle-up" + sizeDefault);
        else setIcon("far fa-angle-down" + sizeDefault);
        setText(t("Filter"));
        setFloat(" float-left");
        setStartIcon(false);
        break;
  ...
  let button = (
    <Button
      ...
      disableTouchRipple
      className={`${
        isTextAndIcon === true ? classes.button : classes.buttonIcon
      } ${float} ${className}`}
    ...
    >
      <span
        className={
          isTextAndIcon === true ? classes.buttonText : classes.buttonIconText
        }
      >
        {label ? label : text}
      </span>
  ...
  if (visible) {
    button = (
      <ThemeProvider theme={theme}>
        {isTextAndIcon === false ? (
          <Tooltip title={label ? label : text}>{button}</Tooltip>
        ) : (
          button
        )}
      </ThemeProvider>
