...
const TransferList = (props) => {
  ...
  useEffect(() => {
    ...
    const lstItemSelected = _.filter(lstItemAll, (x) =>
      _.includes(itemApllyIds, x.id),
    );
    const lstItemNotSelected = _.filter(
      lstItemAll,
      (x) => !_.includes(itemApllyIds, x.id),
    );
  ...
  const onSwitchRight = (event) => {
    ...
    if (!_.isEmpty(itemSelectedIds)) {
      const lstItemSelected = _.filter(lstItemAll, (x) =>
        _.includes(itemSelectedIds, x.id),
      );
      const lstItemNotSelected = _.filter(
        lstItemAll,
        (x) => !_.includes(itemSelectedIds, x.id),
      );
      ...
      const lstIdApply = _.map(lstItemApply, (x) => {
        return x.id;
      });
      onChange &&
        onChange({ value: lstIdApply, name, type: EControlType.transferList });
  ...
  const onSwitchLeft = (event) => {
    ...
    if (!_.isEmpty(itemSelectedIds)) {
      const lstItemSelected = _.filter(lstItemApply, (x) =>
        _.includes(itemSelectedIds, x.id),
      );
      const lstItemNotSelected = _.filter(
        lstItemApply,
      ...
      const lstIdApply = _.map(lstItemNotSelected, (x) => {
        return x.id;
      });
      onChange &&
        onChange({ value: lstIdApply, name, type: EControlType.transferList });
  ...
  const renderAllItem = () => {
    return (
      <Grid container gap={2} direction="column">
        ...
        >
          <CheckBoxControl
            disabled={_.isEmpty(lstItemAll) ? true : false}
              ...
              return (
                ...
                >
                  <Grid item>
                    <CheckBoxControl
                      onChange={({ value }) =>
                        onChangeSelectedItemNotApply(value, item.id)
                      }
                      control={control}
                      name={item.id}
                    />
  ...
  const renderItemApply = useCallback(() => {
    return (
      <Grid container gap={2} direction="column">
        ...
        >
          <CheckBoxControl
            disabled={_.isEmpty(lstItemApply) ? true : false}
            onChange={onCheckAllItemApply}
            control={control}
            name="all-apply"
            label={t("All")}
          />
        </Grid>
        {_.isEmpty(lstItemApply)
          ? renderEmptyRow()
          : _.map(lstItemApply, (item) => {
              return (
                <Grid
                ...
                >
                  <Grid item>
                    <CheckBoxControl
                      onChange={({ value }) =>
                        onChangeSelectedItemApply(value, item.id)
                      }
                      control={control}
                      name={`${item.id}-apply`}
                    />
      ...
      >
        <Grid item>
          <ButtonIcon
            ...
            disabled={disabledSwitchRight}
            type={EButtonIconType.switchRight}
          />
        </Grid>
        <Grid item>
          <ButtonIcon
            ...
            disabled={disabledSwitchLeft}
            type={EButtonIconType.switchLeft}
          />
        </Grid>
      </Grid>
      <Grid item xs className="body-content">
