...
const HeaderCell = (props) => {
    ...
    const renderHeaderIcon = () => {
        if (item.type === EControlType.select) {
            return <ButtonIcon className='cell-icon' onClick={() => item.onRefersh && item.onRefersh()} size={ESize.small} type={EButtonIconType.refresh} />
        }
        return ''
    }
    return <TableCell className='cell-header' key={item.name} style={{ minWidth: item.width }}>
        {item.title}
