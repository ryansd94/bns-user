...
const GridControl = (props) => {
    ...
    const renderButton = useCallback(() => {
        return <ButtonFuntion onClick={onAddRow} type={EButtonType.add} label={addButtonLabel} />
    })
    ...
    return <Grid container gap={2} direction='column' className="no-wrap">
        ...
        <GridValidation />
        <Grid item container direction='column' className="no-wrap of-hidden bs-grid">
            <Paper sx={{ width: '100%', overflow: 'hidden', display: 'flex' }}>
                <TableContainer component={Paper}>
                    <Table id={id} stickyHeader sx={{ minWidth: 650 }} aria-label="sticky table">
                        <TableHead>
                            {renderHeader()}
                        </TableHead>
                        <TableBody>
