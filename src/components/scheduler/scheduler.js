...
class Scheduler extends Component {
    ...
    render() {
        ...
        let radioButtonList = config.views.map(item => {
            return <button onClick={() => this.onViewChange(item.viewType, item.showAgenda ? 1 : 0, item.isEventPerspective ? 1 : 0)} key={`${item.viewType}${item.showAgenda ? 1 : 0}${item.isEventPerspective ? 1 : 0}`}>{item.viewName}</button>
        })
        ...
        if (config.headerEnabled) {
            schedulerHeader = (
                ...
                <Grid item container alignItems={'center'} justifyContent='space-between' style={{ marginBottom: '24px' }}>
                    {leftCustomHeader}
                    <Grid item>
                        <div className='header2-text'>
                            <IconLeft style={{ marginRight: "8px" }} className="icon-nav"
                                onClick={this.goBack} />
                            <span className={'header2-text-label'}>{dateLabel}</span>
                            <IconRight style={{ marginLeft: "8px" }} className="icon-nav"
                                onClick={this.goNext} />
                        </div>
                    </Grid>
                    <Grid item>
