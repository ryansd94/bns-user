...
function TabPanel(props) {
  ...
  return (
    ...
    >
      ...
      {renderAllAtFirstLoad === true ? <div>{children}</div> : ""}
    </div>
  );
}
...
const TabControl = (props) => {
  const {
    tabItems = [],
    classNameSwipeableView,
    id,
    renderAllAtFirstLoad = false,
  } = props;
  const indexDefault = _.findIndex(tabItems, (x) => x.isActive);
  const [value, setValue] = React.useState(
    indexDefault != -1 ? indexDefault : 0,
  );
    ...
    >
      <AntTabs value={value} onChange={handleChange} aria-label="ant example">
        {_.map(tabItems, (item, index) => {
          ...
          if (_.includes(errorTabs, index)) {
            return (
              <AntTab
                icon={<IconRequire className="icon-require-tab" />}
                iconPosition="end"
                key={index}
                label={item.label}
              />
      ...
      >
        {_.map(tabItems, (item, index) => {
          return (
            <TabPanel
              renderAllAtFirstLoad={renderAllAtFirstLoad}
              key={index}
              value={value}
              index={index}
            >
