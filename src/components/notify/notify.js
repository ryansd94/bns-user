...
const Notify = (props) => {
  ...
  const renderNotifyItem = (item) => {
    ...
    return (
      <TaskCommentNotify
        ...
        key={item.id}
        data={data}
        type={ENotifyComponentType.popover}
      />
    );
  };
  ...
  const handleScroll = async () => {
    ...
    const scrollPosition =
      notifyContainerRef.current.scrollTop +
      notifyContainerRef.current.offsetHeight;
      ...
      baseUrl.jm_notifyUser,
      {
        start:
          isGetMore === true
            ? currentPage
              ? currentPage *
                (_.isNil(lengthRowLoadMore) ? 10 : lengthRowLoadMore)
              : 0
            : 0,
  ...
  const renderNotifyPopup = () => {
    ...
    const lstNotifyToday = _.filter(
      notifyItems,
      (x) =>
        new Date(x.createdDate).getDate() === currentDate.getDate() &&
        new Date(x.createdDate).getMonth() === currentDate.getMonth() &&
        new Date(x.createdDate).getFullYear() === currentDate.getFullYear(),
    );

    const lstNotifyBefore = _.filter(
      notifyItems,
      (x) => new Date(x.createdDate) < today,
    );
    ...
    return (
      <Grid container item xs direction={"column"} className="no-wrap">
        <Grid item>
          ...
          >
            ...
            <Grid container item xs gap={2}>
              <Grid item>
                <span
                  onClick={() => onChangeNotifyAction()}
                  className={`notify-action ${isAll !== false ? "active" : ""}`}
                >
                  {t("All")}
                </span>
              </Grid>
              <Grid item>
                <span
                  onClick={() => onChangeNotifyAction(false)}
                  className={`notify-action ${isAll === false ? "active" : ""}`}
                >
                  {t("Unread")}
                </span>
        ...
        <Grid item className="flex of-hidden">
          {loading === true ? (
            <NotifyLoading />
          ) : !_.isEmpty(notifyItems) ? (
            <div
              ref={notifyContainerRef}
              onScroll={handleScroll}
              className="notify-tab-body no-wrap flex-grow"
            >
              <Grid container item xs gap={2} direction={"column"}>
                {!_.isEmpty(lstNotifyToday) ? (
                  <Grid container item xs direction={"column"}>
                    <Grid item className="notify-group-title">
                      {t("Today")}
                    </Grid>
                    <Grid
                      item
                      container
                      className="notify-group-content"
                      xs
                      gap={1}
                    >
