...
const Sidebar = (props) => {
  ...
  const getPathItem = (item) => {
    ...
    if (user) {
      ...
      return !_.isNil(defaultOrganization)
  ...
  const renderMenuItem = (item, isChild = false) => {
    switch (item.type) {
      case EMenuType.group:
        ...
        const childKeys = _.map(childGroups, (x) => {
          return _.toLower(x.key)
        })
        const isActiveGroup = _.some(
          viewPermissions,
          (x) =>
            _.includes(childKeys, _.toLower(x.view)) &&
            _.find(
              x.actions,
              (a) => _.isEqual(a.key, constants.view) && a.value === true,
            ),
        )
        return isActiveGroup || user.isMainAccount ? (
          <>
            <li key={item.key} className="nav-item nav-category">
      ...
      case EMenuType.action:
        const isActiveAction = _.some(
          viewPermissions,
          (x) =>
            _.isEqual(_.toLower(x.view), _.toLower(item.key)) &&
            _.find(
              x.actions,
              (a) => _.isEqual(a.key, constants.view) && a.value === true,
            ),
        )
        return isActiveAction || user.isMainAccount ? (
          <li
            key={item.key}
            className={`nav-item ${
              isChild === false
                ? isPathActiveCollapse([item])
                  ? "active"
                  : ""
                : ""
            }`}
          >
            ...
            >
              ...
              <span className={`menu-title ${isChild === true ? "item" : ""}`}>
                {t(item.title)}
              </span>
            </Link>
          </li>
      ...
      case EMenuType.collapse:
        const childCollapses = deepFindAll(
          item.childs,
          function (obj) {
            return _.isEqual(_.toLower(obj.parent), _.toLower(item.key))
          },
        ...
        const childCollapseKeys = _.map(childCollapses, (x) => {
          return _.toLower(x.key)
        })
        const isActiveCollapse = _.some(
          viewPermissions,
          (x) =>
            _.includes(childCollapseKeys, _.toLower(x.view)) &&
            _.find(
              x.actions,
              (a) => _.isEqual(a.key, constants.view) && a.value === true,
            ),
        )
        return isActiveCollapse || user.isMainAccount ? (
          <li
            key={item.key}
            className={
              isPathActiveCollapse(item.childs) ? "nav-item active" : "nav-item"
            }
          >
            <div
              aria-expanded={getAriaExpanded(item.key)}
              className={
                getAriaExpanded(item.key)
                  ? "nav-link menu-expanded"
                  : "nav-link"
              }
              onClick={() => toggleMenuState(item.key)}
  ...
  const onProjectChange = (value) => {
    ...
    if (!_.isNil(item)) {
      save(`${baseUrl.jm_user}/me`, {
        id: userInfo.userId,
        configs: [
          { key: "ProjectSetting.Current", value: item.code },
          { key: "ProjectSetting.CurrentId", value: item.id },
        ],
      })
      ...
      dispatch(
        setUserSetting({
          setting: {
            projectSetting: { current: item.code, currentId: item.id },
          },
        }),
      )
      setUserInfo({ user: userInfo })
      history.push(
        `/${userInfo.defaultOrganization.code}/${item.code}/overview/summary`,
      )
    }
  }
