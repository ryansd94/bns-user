...
const ToolBar = (props) => {
  ...
  let [visibleObject, setVisibleObject] = useState(
  ...
  const onChangeVisibleToolbar = ({ id, isEnableButtonFunction, selectedIds }) => {
    if (id === gridId) {
      visibleObject.function = isEnableButtonFunction
  ...
  const renderDropdownItem = () => {
    return (
      <Grid container item direction={"column"}>
        {_.includes(listPermission, EButtonType.delete) ? (
          <DropDownItem onClick={onDeleteRowsSelectedClick} title={t("Delete")} icon={<IconDelete />} />
        ) : (
          ""
        )}
      </Grid>
  ...
  return (
    <Grid
      ...
      xs
      direction={"column"}
      className="no-wrap fg-initial body-content-item"
    >
      <Grid item xs>
        <ConfigColumn
          ...
          handleClose={handleCloseColumn}
        />
        <ButtonFuntion
          spacingLeft={0}
          visible={visibleObject.column}
          onClick={handleClickColumn}
          type={EButtonType.columnConfig}
        />
        <ButtonFuntion
          spacingLeft={1}
          visible={visibleObject.column}
          ...
          type={EButtonType.filter}
        />
        {!_.isEmpty(listPermission) ? (
          <DropdownMenu
            isShowEndIcon={false}
            isCloseOnClick={true}
            visible={visibleObject.function}
            type={EButtonType.function}
        ...
        <ButtonFuntion
          spacingLeft={1}
          visible={visibleObject.add}
          onClick={onAddClick}
          type={EButtonType.add}
        />
        <ButtonFuntion
          spacingLeft={1}
          visible={visibleObject.delete}
