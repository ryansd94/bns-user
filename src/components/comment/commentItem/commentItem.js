...
import { getUserName, formatDistanceDate } from "helpers";
...
const CommentItem = (props) => {
  ...
  const renderChildComment = (comment) => {
    if (!_.isEmpty(comment.childrens)) {
      return (
        <Grid id={comment.id} item container gap={2} direction="column">
  ...
  const renderMoreChildLink = () => {
    return (
      <Grid item xs style={{ paddingLeft: `${commentLocal.level * 60}px` }}>
        <LinkControl
          underline="none"
          onClick={onShowMoreComment}
          title={`Hiển thị ${commentLocal.countReply} trả lời`}
        />
      </Grid>
    );
  };
  ...
  const renderListComment = () => {
    if (!_.isNil(taskId)) {
      if (commentLocal.countReply > 0) {
        return (
          <div>
            {renderChildComment(commentLocal)}
            {renderMoreChildLink()}
          </div>
        );
      } else {
  ...
  const renderPopoverControl = () => {
    return (
      <Grid item container gap={2} className="box-container">
        <Grid item>{t("Do you want to delete this comment?")}</Grid>
  ...
  const onConfrimChangeComment = async (value) => {
    ...
    const res = await save(`${baseUrl.jm_comment}`, {
      id: commentLocal.id,
        ...
        >
          <Grid item>
            <LabelControl
              ...
              label={`${commentLocal.user?.fullName}, ${formatDistanceDate(
                commentLocal.createdDate,
              )}`}
            />
          </Grid>
          <Grid item id={comment.id}>
            <EditorControl
              control={control}
              className={!isEditComment ? "editor-comment" : ""}
              readOnly={!isEditComment}
              name={controlId}
          ...
          <Grid item>
            <CommentFooter
              ...
              {...props}
              id={commentLocal.id}
              onConfrimChangeComment={handleSubmit(onConfrimChangeComment)}
  ...
  return !_.isEmpty(commentLocal) ? (
    <Grid direction={"column"} item xs container gap={2}>
      <Grid
        className="comment-component"
        style={{
          position: "relative",
          paddingLeft: `${commentLocal.level * 60}px`,
        }}
        item
        container
        spacing={2}
      ...
      >
        ...
        {!commentLocal.isDelete && user.userId === comment.createdUserId ? (
          <DropdownMenu
            classNameIcon="icon-more-comment"
            renderDropdownItem={renderDropdownItem}
            className={`dropdown-more-comment ${
              !_.isNil(openPopover) ? "dropdown-more-comment-show" : ""
            }`}
            isButtonIcon={true}
        ...
        <PopoverControl
          className="pop-delete-comment"
          isHideWhenWithOutFocus={false}
