...
const Filter = (props) => {
  const {
    ...
    component,
    isSaveFilter = true,
    isChangeUrlWhenApplyFilters = true,
    filterModels,
  } = props;
  const open = Boolean(anchorEl);
  const [anchorSave, setAnchorSave] = React.useState(null);
  const [filterData, setFilterData] = React.useState(null);
  const { control, reset, handleSubmit, setValue, getValues, setError } =
    useForm({});
  ...
  const onSave = async (data) => {
    ...
    if (value) {
      const res = await save(baseUrl.sys_filter, {
        name: data.name,
        filterData: JSON.stringify(value),
        component: component,
      });
    }
  };
        ...
        if (item.column && item.condition != null) {
          if (!item.selectValue) {
            ...
            value.push({
              column: item.column,
              condition: item.condition,
              value: item.selectValue.map((e) => e).join(","),
              type: item.type,
              isCustom: item.isCustom,
            });
          }
  ...
  return (
    <Collapse className="containerNew" in={open} timeout="auto" unmountOnExit>
      <Box className="box-container">
        <Grid width="auto" container direction="column" gap={2}>
          <Grid key="header" container gap={2} item>
            <Grid item>
              <ButtonFuntion
                spacingLeft={0}
                onClick={onAdd}
                type={EButtonType.addFilter}
              />
            </Grid>
            {!isSaveFilter ? (
              ""
          ...
          {fields.map((item, index) => {
            return (
              <FilterItem
                onClearConditionValue={onClearConditionValue}
          ...
          <Grid key="footer" style={{ display: "flex" }} item xs={12}>
            <ButtonFuntion
              spacingLeft={0}
              onClick={onClear}
              type={EButtonType.clearFilter}
            />
            <ButtonFuntion
              style={!isSaveFilter ? { marginLeft: "auto" } : {}}
              spacingLeft={2}
