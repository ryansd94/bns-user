...
const TaskCommentNotify = React.forwardRef(
  function TaskCommentNotify(props, ref) {
    const {
      data = {},
      id,
      type = ENotifyComponentType.snackbar,
      onClickNotify,
    } = props;
    ...
    const renderTaskItem = (classContent) => {
      return (
        <Grid container gap={1} flexWrap="nowrap">
          {!_.isEmpty(content.TaskContent.TaskType.Icon) ? (
            <Grid item>
              <LabelIconControl
                icon={content.TaskContent.TaskType.Icon}
                color={content.TaskContent.TaskType.Color}
              />
            </Grid>
          ) : (
            ""
    ...
    const renderContent = (isRenderPopover = false) => {
      ...
      return (
        <Grid gap={1} container direction={"column"}>
          <Grid item>
            <span className={`notify-mention ${classContent}`}>
              {content.UserMention.FullName}
            </span>
            <span className={`notify-title ${classContent}`}>
              {getTitleNotify()}
            </span>
          </Grid>
          <Grid item>{renderTaskItem(classContent)}</Grid>
          {isRenderPopover === true ? (
            <Grid item>{getFormatDateDistance(notify.createdDate)}</Grid>
          ) : (
            ""
          )}
        </Grid>
    ...
    const getTaskUrl = () => {
      return getPathItem(
        `/task/edit/${content.TaskContent.Id}?viewId=${notify.objectId}`,
      );
    };
    ...
    const renderContentPopover = () => {
      return (
        <Grid
          ...
          xs
          gap={2}
          className="no-wrap align-items-center"
          justifyContent={"space-between"}
        >
          <Grid item>
            <AvatarControl
              name={content.UserMention.FullName}
              image={content.UserMention.Image}
            />
          </Grid>
    ...
    const renderComponent = () => {
      if (type === ENotifyComponentType.snackbar) {
        ...
        return (
          <Grid container gap={2} className="notify-list-container">
            <Grid item xs className="notify-list-item">
              <LinkControl
                onClick={onNotifyClick}
                className="notify-link"
                href={getTaskUrl()}
                title={renderContentPopover()}
              />
            </Grid>
          </Grid>
        );
