...
const StatusTemplate = React.memo((props) => {
  ...
  const { setValue, id, onValueChange, control, statusData, getValues, name } =
    props;
  const status = _.cloneDeep(getValues(name));
  const getListStatusTemplate = () => {
    if (_.isNil(statusData) || _.isEmpty(statusData)) return [];
    return _.map(
      _.filter(statusData, (x) => x.isAutomaticAdd === true),
      (y) => {
        return { id: y.id };
      },
    );
  };
  const originData = !_.isNil(status)
    ? _.cloneDeep(status)
    : statusData && getListStatusTemplate();
  const [listStatusTemplate, setListStatusTemplate] = useState(originData);
  const listStatusDataIds = _.map(statusData, (x) => {
    return x.id;
  });

  const getListStatusTemplateRemaining = () => {
    const currentListStatusIds = _.map(
      _.filter(listStatusTemplate, (item) => item.isCurrentSelected !== true),
      (x) => {
        return x.id;
      },
    );
    const remainingListStatus =
      _.difference(listStatusDataIds, currentListStatusIds) || [];
    return remainingListStatus;
  };
  const [listStatusTemplateRemaining, setListStatusTemplateRemaining] =
    useState(getListStatusTemplateRemaining());
  const {} = useFieldArray({
    control,
    name: name,
  });
  ...
  const onStatusDelete = (deletedItem) => {
    const updateStatus = listStatusTemplate.filter(
  ...
  const addNew = () => {
    ...
    const currentListStatusIds = _.map(currentListStatus, (x) => {
      return x.id;
    });
    const remainingListStatus = _.difference(
      listStatusDataIds,
      currentListStatusIds,
    );
    if (!_.isEmpty(remainingListStatus)) {
      setListStatusTemplateRemaining([...remainingListStatus]);
      currentListStatus.push({
        id: remainingListStatus[0],
  ...
  const renderStatusItem = (statusItem, index) => {
    if (statusItem.isDelete) return "";
    let options = [];
    const disabled = statusItem.isCurrentSelected !== true ? true : false;
    if (statusItem.isCurrentSelected !== true) {
      options = statusData;
    } else {
      options = _.filter(statusData, (x) =>
        _.includes(listStatusTemplateRemaining, x.id),
      );
    }
  ...
  return (
    <Grid container gap={2} direction="column" item>
      ...
      <Grid container gap={2} direction="column" item>
        {!_.isNil(listStatusTemplate) &&
          listStatusTemplate.map((item, index) => {
            const statusItem = _.find(statusData, (x) => x.id === item.id);
            if (_.isNil(statusItem)) return "";
            return renderStatusItem(
              { ...statusItem, isCurrentSelected: item.isCurrentSelected },
              index,
            );
          })}
      </Grid>
