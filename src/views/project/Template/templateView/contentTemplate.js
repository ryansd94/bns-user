...
const ContentTemplate = (props) => {
  ...
  const onDragEndTitle = (result) => {
    ...
    if (destinationDroppableId.indexOf("@") != -1) {
      ...
      if (
        destinationGroup &&
        destinationGroup.type === EControlType.group &&
        removedElement.type === EControlType.group
      )
        return;
    }
    ...
    if (!_.isNil(id)) {
      onValueChange({ value: listCopy, name, isEntity: false });
    }
  ...
  const onAddControlSubmit = (data, index, prefix, item, field) => {
    ...
    listCopy[prefix] = addToList(destinationList, positionIndex, prefixGroup, {
      id: id,
      type: data.type,
      label: title,
          ...
          <Grid className="flex-container" container gap={2} item xs={12}>
            <Grid item>
              <AssignSelect
                ...
                name={"assign"}
                data={[
                  { id: 1, name: t("Assigner 1") },
                  { id: 2, name: "Assigner 2" },
                  { id: 3, name: "Assigner 3" },
                ]}
              />
            </Grid>
            <Grid item>
              ...
              <DragDropContext onDragEnd={onDragEndTitle}>
                ...
                >
                  <Grid
                    ...
                    item
                    xs={12}
                    sm={getElementControls("column3").length > 0 ? 6 : 9}
                  >
                    <DraggableElement
                      field={field}
