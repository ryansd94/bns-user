...
const TooltipControl = React.memo((props) => {
    ...
    return (
      <SettingControl
        onApply={(data, index, prefix, item) =>
          handleSettingClose(data, index, prefix, item)
        }
        prefix={prefix}
        index={index}
        item={item}
  ...
  return (
    <Grid container gap={2} style={{ padding: "1rem" }}>
      <Grid item>
        <ButtonIcon
      ...
      <Grid item>
        <ButtonIcon
          ...
          onClick={() => onAction(EButtonIconType.up)}
          type={EButtonIconType.up}
        />
      </Grid>
      <Grid item>
        <ButtonIcon
          color="primary"
          disabled={isLastControl ? true : false}
          onClick={() => onAction(EButtonIconType.down)}
          type={EButtonIconType.down}
        />
      </Grid>
      <Grid item>
        <ButtonIcon
          color="error"
          onClick={() => onAction(EButtonIconType.delete)}
      ...
      <Grid item>
        <ButtonIcon
          ...
          type={EButtonIconType.setting}
        />
        <PopoverControl
          isCLoseOnHover={true}
          genderBody={renderSettingControl}
