...
const MemberTab = React.memo((props) => {
    ...
    const customFilterData = (data) => {
        ...
        if (!_.isEmpty(users)) {
            const usersSelectedIds = _.map(users, (x) => { return x.userId })
    ...
    const onSelectedRowsChange = (rows) => {
        let users = _.cloneDeep(getValues(name))
        users = [...users, ...rows]
        onValueChange && onValueChange({ value: users, name, isEntity: false, type: EControlType.listId })
        setValue(name, users)
        if (!_.isEmpty(users)) {
            eventEmitter.emit('onRowDataChange', { rows: users, gridId: 'gridTeamUser' })
        }
    }
    ...
    const handleDeleteMulti = (dataSelect) => {
        if (!_.isEmpty(usersSelected)) {
            ...
            onValueChange && onValueChange({ value: result, name, isEntity: false, type: EControlType.listId })
    ...
    const renderGridSelect = useCallback((users) => {
        return <GridSelect
            filterModels={filterModels}
