...
const ViewPermissionApplyUser = (props) => {
    ...
    const renderAllUsers = () => {
        return <Grid container gap={2} direction='column'>
            <Grid item container direction='row' className="border-bottom">
                <Grid item>
                    <CheckBoxControl disabled={_.isEmpty(lstUserAll) ? true : false} onChange={onCheckAllUser} control={control} name='all' label={t('All')} />
                </Grid>
            </Grid>
            {
    ...
    const renderUserApply = useCallback(() => {
        return <Grid container gap={2} direction='column'>
            <Grid item container direction='row' className="border-bottom">
                <Grid item>
                    <CheckBoxControl disabled={_.isEmpty(lstUserApply) ? true : false} onChange={onCheckAllUserApply} control={control} name='all-apply' label={t('All')} />
                </Grid>
            </Grid>
            {
    ...
    return <Grid container direction='row'>
        <Grid item xs className='body-content'>
            <Group content={renderAllUsers()} title={t('User list')} />
        </Grid>
        ...
        <Grid item xs className='body-content'>
            <Group content={renderUserApply()} title={t('List of applicable users')} />
        </Grid>
    </Grid>
}
