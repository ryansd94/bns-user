...
export default function Login() {
  ...
  const onLoginGoogleSuccess = async (user) => {
    ...
    switch (res.status) {
      case httpStatus.OK: {
        ...
        if (data.errorCode == ERROR_CODE.userNotRegister) {
          history.push(
            `/signup?token=${data.data.token}&googleToken=${user.accessToken}`,
          )
          break
        } else if (data.errorCode == ERROR_CODE.userWaitingConfirm) {
          history.push(
            `/jointeam?accountCompanyId=${data.data.accountCompanyId}`,
          )
          break
          ...
          <div className="col-lg-4 mx-auto">
            <div className="auth-form-light text-left py-5 px-4 px-sm-5">
              ...
              <Grid container gap={2} direction="column">
                ...
                <Grid item xs>
                  <div className="mb-2" onClick={() => signInWithGoogle(onLoginGoogleSuccess)}>
